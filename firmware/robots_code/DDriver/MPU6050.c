...
short angle_z_send;
float angle_z,Deviation_gyro_z_mean,angle_HMC;
float dt=0.01;
int Deviation_gyro_z,Temp;
float gyro_z,angle_HMC_real;
    ...
    {	
     ...
     MPU_Get_Accelscope(); //获得加速度计值(原始值)
		...
		if(Deviation_Count<CONTROL_DELAY)
		  {	 
				if(Deviation_Count>199){
		  	Deviation_gyro_z=Deviation_gyro_z+(int)gyro[2];
				if(Deviation_Count==999) Deviation_gyro_z_mean=0.1*Deviation_gyro_z*10/800;
				}
				Deviation_Count++;
			...
			HMC_READ();
				MHC_flag=0;
						 if(((angle_HMC_offset+angle_HMC)>=-180)&&((angle_HMC_offset+angle_HMC)<=180))
						 angle_HMC_real=angle_HMC_offset+angle_HMC;
						 else if ((angle_HMC_offset+angle_HMC)>180)
							 angle_HMC_real=angle_HMC_offset+angle_HMC-360;
						 else if ((angle_HMC_offset+angle_HMC)<-180)
								angle_HMC_real=angle_HMC_offset+angle_HMC+360;
...
u8 MPU6050_initialize(void) 
  ...
  MPU6050_setFullScaleAccelRange(MPU6050_ACCEL_FS_2);
	MPU6050_Set_Rate(100);			//Set the sampling rate to 50Hz //设置采样率50Hz
	...
	if(res==MPU6050_DEFAULT_ADDRESS) //The device ID is correct, The correct device ID depends on the AD pin //器件ID正确, 器件ID的正确取决于AD引脚
	{
		...
		MPU6050_Set_Rate(100);	                      //Set the sampling rate to 50Hz //设置采样率为50Hz   
 	}else return 1;
	return 0;
...
void MPU_Get_Gyroscope(void)
{
	...
	if(Deviation_Count<CONTROL_DELAY) // 10 seconds before starting //开机前10秒
		...
		{  
			...
			gyro_z=(float)gyro[2]-Deviation_gyro_z_mean;
			if(angle_HMC_real>180)angle_HMC_real=180;
			if(angle_HMC_real<-180)angle_HMC_real=-180;
			if((angle_z-angle_HMC_real)>90)angle_z=angle_HMC_real;
			if((angle_z-angle_HMC_real)<-90)angle_z=angle_HMC_real;
			
			angle_z=0.999*(angle_z+(float)(0.0152587890625*0.5*gyro_z)/RATE_100_HZ)+0.001*angle_HMC_real; //角度制
			if(angle_z>180) angle_z=angle_z-360;
			if(angle_z<-180) angle_z=angle_z+360;
			angle_z_send=(short)(angle_z*100); 
